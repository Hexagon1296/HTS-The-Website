...
let music = new Audio("Ryzmik-Permafrost.mp3");
music.loop = true;
music.play();
...
canvas.addEventListener("click",function(){
  if(qNum==-1||qNum==25){
    ...
    if(qNum==25){
    let textWidth = ctx.measureText("EXPLANATIONS AND CREDITS").width;
...
setInterval(function(){
  ...
  if(qNum==-1||qNum==25){
    ...
    ctx.font = "80px serif";
    ctx.textAlign = "center";
    ctx.fillText(qNum==25?"YOU WIN!":"INCORRECT!",canvas.width/2,90)
    ctx.fillStyle = "green";
    ctx.font = "40px serif";
    let textWidth = ctx.measureText("TRY AGAIN?").width;
    ...
    ctx.strokeText("EXPLANATIONS AND CREDITS",canvas.width/2,420+ctx.letterSize/2)
    ctx.fillText("EXPLANATIONS AND CREDITS",canvas.width/2,420+ctx.letterSize/2)
    }
    ctx.fillStyle = "white";
    ctx.beginPath();
